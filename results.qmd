...
We first investigate the rise in crimes across the NY State counties. Since there are 62 counties in the state, we focus on the top 10 counties that have experienced the most significant percentage increase in crime since 2020. We refer to the [interactive graph](#interactive-ts), which allows to get a more comprehensive view over the data. In particular, it is useful to contrast the COVID-19 impact among different counties subdivided by time-interval and crime categories.
...
ggplot(crime_top_10, 
  ...
  labs(title = "Crime Rates in Select Counties post-2020",
       x = "Year",
       y = "Crime (per 1,000 residents)") +
  theme_minimal()
...
ggplot(crime_top_bot_5, 
  ...
  geom_point() + 
  scale_color_manual(values = combined_palette) +
  labs(title = "Crime for Counties with Least/Most Increase After 2020",
       x = "Year",
       y = "Crime (per 1,000 residents)") +
  theme_minimal()
...
ggplot(crime_types_long, aes(x = Year, 
  ...
  labs(title = "Crime Types in Schuyler Since 2015",
       x = "Year",
...
#           position = "stack") +
  geom_area(position = 'stack',
            alpha = 0.7) +
  labs(title = "Crime Types by Region since 2000",
       x = "Year",
       y = "Crime (per 1,000 residents)") +
  theme_minimal() +
  facet_wrap(~Location, 
...
The analysis of the evolution of each crime type was supported by the use of the two [interactive plots](#interactive-overall), which allowed us to subdivide and explore the data in much more detail.
...
# we remove some labels by hand to avoid overplotting
repl_some_county_names <- c("Cortland", "Cayuga","Chenango","Genesee",
                            "Clinton","Steuben", "Schenectady", "Chautauqua", "Tompkins",
                            "Franklin","Oswego","Herkimer", "St Lawrence", "Livingston",
                            "Tioga", "Ulster", "Wyoming", "Orleans", "Schoharie", 
                            "Yates", "Hamilton", "Madison", "Essex", "Seneca", 
                            "Ontario", "Washington", "Otsego", "Wayne", "Chamung", "Schuyler", 
                            "Rensselaer", "Oneida", "Cattaraugus")
crime_ny_filtered_county_tot_since_1990 <- crime_ny_filtered_county_tot_since_1990 %>%
  mutate(County = ifelse(County %in% repl_some_county_names, ' ', County))
...
draw_biplot(crime_ny_filtered_county_tot_since_1990[,c(1,3:9)], 
            ...
            point_labels = T, 
            point_size = 1) +
  labs(title = 'Biplot of Crime Rate (per 1,000 Residents) from 1990 to 2022') +
  theme_minimal()
```
...
# we remove some labels by hand to avoid overplotting
repl_some_county_names <- c("Cortland", "Cayuga","Genesee",
                            "Clinton","Steuben", "Tompkins",
                            "Oswego","Herkimer", "St Lawrence", "Lewis",
                            "Tioga", "Ulster", "Wyoming", "Orleans", "Schoharie", 
                            "Yates", "Madison", "Essex", "Seneca", 
                            "Ontario", "Washington", "Otsego", "Wayne", "Chamung", 
                            "Oneida", "Cattaraugus", "Allegany", "Greene")
crime_ny_filtered_county_tot_since_2010 <- crime_ny_filtered_county_tot_since_2010 %>%
  mutate(County = ifelse(County %in% repl_some_county_names, ' ', County))
...
draw_biplot(crime_ny_filtered_county_tot_since_2010[,c(1,3:9)], 
            ...
            point_labels = T, 
            point_size = 1) +
  labs(title = 'Biplot of Crime Rate (per 1,000 Residents) from 2010 to 2022') +
  theme_minimal()
...
It is interesting to contrast the viewpoint offered by these graphs with the [interactive barchart](#interactive-bar), which allows us to compare the crime-incident numbers by categories across years and counties and provides a somewhat orthogonal viewpoint.
...
draw_biplot(crime_ny_filtered_county_tot_since_2020_NY[,c(1,3:9)], 
            ...
            point_size = 1,
            'Robbery') +
  labs(title = 'Biplot of Crime Rate (per 1,000 Residents) from 2020 to 2022 in NYC') +
  theme_minimal()
```
...
ggplot(combined_NYC, 
  ...
  labs(title = "NYC vs Non-NYC",
       x = "Year",
       y = "Incidents per (1000 residents)",
       color = "Area") +
...
The "stop-and-frisk" policy, which was part of Operation Impact, allows police officers to stop, question, and search individuals they suspect of criminal activity. The practice is often associated with Bloomberg's time as Mayor of NYC (2002-2013). It was especially prevalent around 2011 where over 685,000 stops were [recorded](https://www.nyclu.org/en/stop-and-frisk-data). The practice was eventually declared unconstitutional and racially biased in the court case of Floyd v. City of New York in 2013. Following Mayor Bill de Blasio's election in 2014, the frequency of stop-and-frisk plummeted.

While there are many in-depth analyses discerning the efficacy of stop-and-frisk (see for instance the scholarly articles, ["The Effects of Local Police Surges on Crime and Arrests in New York City," by
J. MacDonald, J. Fagan, and A. Geller](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0157223) and ["Reforming stop-and-frisk," by H.F. Fradella, and M.D. White](https://www.scopus.com/record/display.uri?eid=2-s2.0-85059953474&origin=inward&txGid=494ff22f99d3709b48481e1aca9aac75)), we believe that our dataset provides an interesting, alternative perspective allowing us to apply various techniques to assess its impact.
...
ggplot(combined_periods, aes(x = Year, y = adjusted_value, color = Location, group = Location)) +
  ...
  labs(title = "NYC vs non-NYC Crime Rates over Different Periods",
       x = "Year",
       y = "Adjusted Crime Rate (per 1000 residents, normalized)") +
  scale_color_manual(values = c("NYC" = "blue", "Non-NYC" = "red")) +
  theme_minimal() +
  facet_wrap(~Period, 
             scales = "free_x", 
